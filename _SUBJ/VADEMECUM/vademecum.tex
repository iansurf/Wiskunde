...
\begin{tikzpicture}[scale=1.0, baseline=(m.center)]
  % --- matrix met herhaling van eerste twee kolommen ---
  \matrix (m) [matrix of math nodes,
                nodes in empty cells,
                left delimiter={(}, right delimiter={)},
                column sep=0.8em, row sep=0.8em] {
     a & b & c & a & b \\
     d & e & f & d & e \\
     g & h & i & g & h \\
  };

  % ---- positieve diagonalen (blauw) ----
  \foreach \x/\y in {1/1, 2/1, 3/1} {
    \draw[thick,blue,->] 
      (m-\x-1.center) -- ++(2.05, -2.05);
  }
  % markeer letters in blauw
  \foreach \coord in {m-1-1, m-2-2, m-3-3,
                      m-1-2, m-2-3, m-3-4,
                      m-1-3, m-2-4, m-3-5}{
    \node[blue] at (\coord) {$\bullet$};
  }

  % ---- negatieve diagonalen (rood) ----
  \foreach \x/\y in {1/3, 2/4, 3/5} {
    \draw[thick,red,->]
      (m-\x-3.center) -- ++(-2.05, -2.05);
  }
  % markeer rode punten
  \foreach \coord in {m-3-1, m-2-2, m-1-3,
                      m-3-2, m-2-3, m-1-4,
                      m-3-3, m-2-4, m-1-5}{
    \node[red] at (\coord) {$\bullet$};
  }

  % labels +
  \node[blue, above=6pt] at (m-1-2.north) {\Large $\mathbf{+}$};
  % label −
  \node[red, below=6pt] at (m-3-3.south) {\Large $\mathbf{-}$};
\end{tikzpicture}
