...
\usepackage{tikz}
\usetikzlibrary{positioning,angles,quotes,calc,babel}
...
\begin{tikzpicture}[scale=3.5, font=\footnotesize]
  ...
  % cos30 = sqrt(3)/2 -> 3 in red; sin30 = sqrt(1)/2 -> 1 in red
  \node[above right] at (P30) {$\left(\tfrac{\sqrt{\textcolor{red}{3}}}{2},\,\tfrac{\sqrt{\textcolor{red}{1}}}{2}\right)$};
  ...
  % both sqrt(2)/2 -> 2 in red
  \node[above right] at (P45) {$\left(\tfrac{\sqrt{\textcolor{red}{2}}}{2},\,\tfrac{\sqrt{\textcolor{red}{2}}}{2}\right)$};
  ...
  % cos60 = sqrt(1)/2 -> 1 in red; sin60 = sqrt(3)/2 -> 3 in red
  \node[above right] at (P60) {$\left(\tfrac{\sqrt{\textcolor{red}{1}}}{2},\,\tfrac{\sqrt{\textcolor{red}{3}}}{2}\right)$};
\end{tikzpicture}
\end{minipage}\hfill
...
30^\circ & \tfrac{\sqrt{\textcolor{red}{1}}}{2} & \tfrac{\sqrt{\textcolor{red}{3}}}{2} \\[6pt]
45^\circ & \tfrac{\sqrt{\textcolor{red}{2}}}{2} & \tfrac{\sqrt{\textcolor{red}{2}}}{2} \\[6pt]
60^\circ & \tfrac{\sqrt{\textcolor{red}{3}}}{2} & \tfrac{\sqrt{\textcolor{red}{1}}}{2}
\end{array}
\]
\end{minipage}
\end{center}
...
= \frac{c}{\sin \widehat{C}} \\[6pt]
\text{Cosinusregel:}\quad
& \begin{cases}
a^{2} = b^{2} + c^{2} - 2bc \cos \widehat{A} \\[6pt]
b^{2} = c^{2} + a^{2} - 2ca \cos \widehat{B} \\[6pt]
...
\end{center}
% -----------------------------------------------------------------------------
% --- 4.3 Determinanten (beeld zoals screenshot) -------------------------------
...
% ==== Rij 1: 2x2 en 3x3 + Sarrus (+ / -) =====================================

% Sarrus: plus-diagonalen
\begin{scope}[shift={(6,0.1)}]
  \node at (0,1.3) {\Large $+$};
  \def\cell{0.55}
  \foreach \i in {0,1,2}{
    \foreach \j in {0,1,2}{
      \fill[white] (\j*\cell,-\i*\cell) circle(0.05);
      \draw (\j*\cell,-\i*\cell) circle(0.05);
    }
  }
  \draw[thick] (0,0) -- (\cell,-\cell) -- (2*\cell,-2*\cell);
  \draw[thick] (\cell,0) -- (2*\cell,-\cell);
  \draw[thick] (0,-\cell) -- (\cell,-2*\cell);
\end{scope}

% Sarrus: min-diagonalen
\begin{scope}[shift={(8,0.1)}]
  ...
  \draw[thick] (0,-2*\cell) -- (\cell,-\cell) -- (2*\cell,0);
  \draw[thick] (0,-\cell) -- (\cell,0);
  \draw[thick] (\cell,-2*\cell) -- (2*\cell,-\cell);
\end{scope}

% ==== Rij 2: cofactor/minor -> adjA ===========================================
% linkse grote determinant met a_ij aangeduid
\begin{scope}[shift={(0,-3.2)}]
\node (minorHint) at (3.9,1.1) {\footnotesize \itshape minor $a_{ij}$ (\,$\times$ weglaten)};
% schets matrix met kruisen in rij/kolom van a_ij
\draw[very thick] (3.0,1.05) .. controls (3.4,1.05) and (3.5,1.05) .. (6.0,1.05); % accolade-achtig
\draw[very thick] (3.0,-1.65) -- (3.0,1.05);
\draw[very thick] (6.0,-1.65) -- (6.0,1.05);

% mini-rooster met kruisjes
\foreach \r/\c in {0/0,0/1,0/2,1/0,1/2,2/0,2/1,2/2}{
  \node at (3.45+0.75*\c,0.65-0.75*\r) {$\times$};
}
\node at (3.45+0.75*1,0.65-0.75*1) {$a_{ij}$};

% pijlen-tekst
\node (cofact) at (0,0) {$\displaystyle
\left|\begin{matrix}\cdots&\cdots&\cdots\\[2pt]\cdots&a_{ij}&\cdots\\[2pt]\cdots&\cdots&\cdots\end{matrix}\right|
\;\;\Rightarrow\;\; \textit{cofactor } a_{ij}=A_{ij}=(-1)^{i+j}$};

\node (toAdj) at (6.9,-0.55) {$\Rightarrow$};

% adjugaat
\node (adj) at (9.6,-0.55) {$\displaystyle
\operatorname{adj}A=\Big[\;A_{ij}\;\Big]^{T}$};
\draw[very thick] (8.3,0.6) -- (8.3,-1.7);
\draw[very thick] (11.0,0.6) -- (11.0,-1.7);
\end{scope}

\end{tikzpicture}

% Laplace regel (onderaan in één lijn)
