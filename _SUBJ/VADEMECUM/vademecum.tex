...
\begin{flushleft}
{Dit is een test van een steekproefgemiddelde $\bar{z}$ volgens steekproefgemiddeldeverdeling $X \approx \mathcal{N}(\mu, \frac{\sigma}{\sqrt{n}})$ in de populatie $\mathcal{N}(\mu, \sigma)$}
\end{flushleft}
...
\\
\hline
$\displaystyle H_A : \bar{z} \le g_{-} \,\, \vee \,\, \bar{x} \ge g_{+}$ &
$\displaystyle H_A : \bar{z} \le g_{-}$ &
$\displaystyle H_A : \bar{z} \ge g_{+}$ \\
\hline
\end{tabular}
\end{center}
...
\begin{flushleft}
{Dit is een test op een populatieproportie $\hat{p}$ volgens een binomiaalverdeling $X \approx \mathcal{B}(n,p) \approx \mathcal{N}(np,{\sqrt{n}} . {\sqrt{p(1-p)}})$}
\end{flushleft}
...
\subsection{P-waarde (grafisch voorgesteld)}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Twee-zijdige toets} & \textbf{Links-zijdige toets} & \textbf{Rechts-zijdige toets} \\
\hline
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    height=4cm,
    xtick={-2.4, 2.4},
    xticklabels={$-z_{\text{obs}}$, $z_{\text{obs}}$},
    x tick label style={yshift=-5pt},
    tick label style={font=\footnotesize},
    xtick style={color=black, thick}
  ]
    \addplot [very thick, black] {exp(-x^2/2)/sqrt(2*pi)};
    \addplot [blue, domain=-3:-2.4, fill=blue, opacity=0.3] {exp(-x^2/2)/sqrt(2*pi)} \closedcycle;
    \addplot [blue, domain=2.4:3, fill=blue, opacity=0.3] {exp(-x^2/2)/sqrt(2*pi)} \closedcycle;
    \node at (axis cs:-2.8,0.05) {\small $p/2$};
    \node at (axis cs:2.8,0.05) {\small $p/2$};
  \end{axis}
\end{tikzpicture}
\end{minipage}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot [very thick, black] {exp(-x^2/2)/sqrt(2*pi)};
    \addplot [blue, domain=-3:-2.4, fill=blue, opacity=0.3] {exp(-x^2/2)/sqrt(2*pi)} \closedcycle;
    \node at (axis cs:-2.8,0.05) {\small $p$};
  \end{axis}
\end{tikzpicture}
\end{minipage}
&
