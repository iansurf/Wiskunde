...
\node (box1) [draw, inner sep=2pt, minimum width=10cm] at (0,0) {%
  ...
  \begin{array}{c @{\qquad} c}
    ...
    {\displaystyle
      ...
      \xrightarrow{\text{Sarrus}}\;
      % ---- inline mini-TikZ placed AFTER the arrow ----
      \tikz[scale=0.65, baseline=-0.5ex]{
        \def\cell{0.45}
        % helper: 3x3 dot grid at (ox,oy)
        \foreach \i in {0,1,2}{
          \foreach \j in {0,1,2}{
            \fill[white] (\j*\cell,-\i*\cell) circle(0.04);
            \draw        (\j*\cell,-\i*\cell) circle(0.04);
          }
        }
        % plus block (left)
        \begin{scope}[shift={(0,0)}]
          \node[above=6pt] at (\cell,0) {\scriptsize $\mathbf{+}$};
          \draw[thick] (0,0) -- (\cell,-\cell) -- (2*\cell,-2*\cell);
          \draw[thick] (\cell,0) -- (2*\cell,-\cell);
          \draw[thick] (0,-\cell) -- (\cell,-2*\cell);
        \end{scope}
        % draw a second grid for the minus block
