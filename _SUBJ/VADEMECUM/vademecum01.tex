...
& (a + bi)^n = (a + bi) \cdot (a + bi) \cdot s (a + bi) \\ \hline
...
\hline
\multicolumn{2}{|c|}{\sqrt[n]{{r\left( {\cos \varphi  + i\sin \varphi } \right)}} = \sqrt[n]{r}\left( {\cos \frac{{\varphi  + k \cdot 2\pi }}{n} + i\sin \frac{{\varphi  + k \cdot 2\pi }}{n}} \right)\quad  \wedge \quad k = 0,1, \cdot s ,n - 1} \\
\hline
 \end{array}
\]
...
\subsection{Sinusregel en cosinusregel}
...
\begin{cases}
a^{2} = b^{2} + c^{2} - 2bc \cos \hat A \\[6pt]
b^{2} = c^{2} + a^{2} - 2ca \cos \hat B \\[6pt]
...
% ==============================
% BEGIN: Inhoud geïmporteerd uit Word (automatisch)
% Bron: Formularium volledig klein 39.docx
% ==============================
\clearpage
\section*{Aanvullingen uit het Word-formularium}
\addcontentsline{toc}{section}{Aanvullingen uit het Word-formularium}
\textbf{Formularium}\textbf{ }\textbf{w}\textbf{iskunde}

1	Algebra	3

1.1	Volgorde van bewerkingen	3

1.2	Absolute waard	3

1.3	Machten met gehele exponenten	3

1.4	Machtswortels	4

2	Veeltermen	5

2.1	Vierkantsvergelijking	5

2.2	Merkwaardige producten – ontbinding in factoren.	5

2.3	De euclidische deling	6

2.4	Rekenschema van Horner.	7

3	Complexe getallen	8

4	Goniometrie	9

4.1	De goniometrische cirkel.	9

4.2	Formules uit de goniometrie	9

4.3	Omgekeerde formules van Simpson	12

4.4	Formules van Simpson	12

4.5	Belangrijke goniometrische waarden	12

4.6	Cyclometrische formules	13
...
6	Oppervlakte en inhoud belangrijke ruimtefiguren	15

7	Reële functies	16

8	Analyse	17

8.1	Limieten van rijen	17

8.2	Limieten van functies	17

8.3	Methodes bij het berekenen van limieten van functies	18

8.4	Afgeleiden	20

8.5	Differentialen	20

8.6	Afgeleiden - fundamentele integralen	21

8.7	Partiële integratie	21

8.8	Fundamentele integralen	22

8.9	Formules bij het oplossen van goniometrische integralen	23

8.10	Integralen van rationale functies	24

8.11	Integralen van irrationale functies	25

8.12	Integralen inhoud - lengte	25

8.13	Exponentiële en logaritmische functies	26

8.14	Maclaurinreeksen, hyperbolische functies.	27

9	Differentiaal	28

9.1	Partiële differentiaal	28

9.2	Differentiaal vergelijkingen	28

10	Logica en verzamelingen	29
...
11.3	Determinanten	31

11.4	Stelsels oplossen	32

12	Combinatieleer	33

12.1	Keuzes zonder herhaling	33

12.2	Keuzes met herhaling	33

13	Kans	34

14	Statistiek	35

14.1	Hypothese testen	37

14.2	Z-tabel	40

15	Diversen	41

15.1	Wiskundige symbolen (ISO 31/XI)	41

15.2	Eenheden en hun veelvouden	45

15.3	Griekse alfabet	46

\textit{16}	Het aanpakken van problemen	46
...
\textbf{H}eel \textbf{M}ooie \textbf{W}itte \textbf{V}aatwassers \textbf{D}oen \textbf{O}nze \textbf{A}fwas

\subsection{Absolute waard}
\subsection{Machten met gehele exponenten}
\subsection{Machtswortels}
\subsubsection{Vierkantswortel in}
\subsubsection{N-de machtswortel in}
\subsubsection{-de machtswortel in}
\section{Veeltermen}
\subsection{Vierkantsvergelijking}
\subsection{Merkwaardige producten – ontbinding in factoren.}
\subsection{De euclidische deling}
\subsection{Rekenschema van Horner.}
\section{Complexe getallen}
\textit{Optelling/Aftrekking : }
...
Vergelijking v/e rechte door punt met rico m :

Vergelijking v/e rechte door 2 punten :

Vergelijking v/e rechte door snijpunt met x-as (r,0) en y-as (0,s) :

Hoek tussen twee rechten a,b met rico m1,m2 :

Afstand tussen rechte a->ux+vy+w=0 en P(x1,y1) :
...
\subsection{De ellips}
\subsection{De hyperbool}
\section{Oppervlakte en inhoud belangrijke ruimtefiguren}
\section{Reële functies}
\section{Analyse}
\subsection{Limieten van rijen}
\subsection{Limieten van functies}
\subsection{Methodes bij het berekenen van limieten van functies}
...
\subsection{Afgeleiden - fundamentele integralen}
\subsection{Partiële integratie}
\subsection{Fundamentele integralen}
\subsection{Formules bij het oplossen van goniometrische integralen}
\subsection{Integralen van rationale functies}
\textit{Stelling van }\textit{d’Alembert}\textit{ :}

1) als

2)\textit{ }\textit{Stelling van Jacobi }\textit{(}\textit{opsplitsen in }\textit{partieel}\textit{breuken}\textit{)}\textit{:}

\subsection{Integralen van irrationale functies}
\subsection{Integralen inhoud - lengte}
...
\subsection{Partiële differentiaal}
\subsection{Differentiaal vergelijkingen}
\section{Logica en verzamelingen}
\section{Matrices}
\subsection{Symbolen}
A			matrix A

het element op rij i en in kolom j

cofactor van hetelement op rij i en in kolom j
...
A(B + C) = AB + AC (links distributiviteit)
...
B=C => A.B=A.C en B.A=C.A (A regulier)

\subsection{Determinanten}
\subsection{Stelsels oplossen}
\subsubsection{n vergelijkingen n onbekenden met 	exorpdfstring{$|A|
eq 0$}{|A| != 0}}
\subsubsection{homogene 2x3-stelsels}
\subsubsection{n+1 eerstegraadsvergelijkingen n onbekenden}
\subsubsection{n eerstegraadsvergelijkingen met n onbekenden ……….}
\section{Combinatieleer}
\subsection{Keuzes zonder herhaling}
\subsection{Keuzes met herhaling}
\section{Kans}
\section{Statistiek}
Discrete data : only a limited number of values is possible

Continuous data :  is a variable such that there are possible values between any two values.

\subsection{Schatters, betrouwbaarheidsintervallen}
\subsection{Regressie}
\subsection{Hypothese testen}
\subsubsection{Test van een hypothese over het gemiddelde van een normaalverdeling}
\subsubsection{Test van een hypothese over een populatieproportie}
\subsubsection{Test van een hypothese over het gemiddelde van een normaalverdeling via de P-waarde}
\subsubsection{Test van een hypothese over een populatieproportie via de P-waarde}
\subsubsection{significantieniveau (-waarde) : sociaal 0,05 / medisch 0,01}
...
is geen element van de verzameling

de verzameling door opsomming

de verzameling waar de elementen voldoen aan de eigenschap p(x)
...
de rationale getallen (breuken van )

de reële getallen

de complexe getallen

BA			B behoort tot A (kan er mee samenvallen)

BA			B behoort strikt tot A

BA			samenvoeging van A en B (unie)

BA			doorsnede van A en B (de gemeenschappelijke elementen)

A \textbackslash{} B			A verschil B, wat tot A behoort en niet tot B

het complement van A in het universum U

(a,b)			het geordend paar

een geordend n-tal

AxB			de productverzameling van A en B

\#			rangnummer of aantal

\subsubsection{Logische symbolen}
pq			conjunctie, de beweringen p \textbf{en} q zijn geldig

pq			disjunctie, de bewering p \textbf{of }q is geldig

p			negatie, de bewering p is niet geldig

pq			implicatie, als p dan q

pq			equivalentie, de beweringen p en q zijn gelijkwaardig

(x)			universele kwantor, voor alle elementen geldt

(x)			existentiële kwantor, er zijn elementen die voldoen aan

\subsubsection{Diverse symbolen}
a=b			a is gelijk aan b

ab			a is niet gelijk aan b

a is per definitie gelijk aan b

ab			a is bij benadering gelijk aan b

a<b			a is strikt kleiner dan b

a>b			a is strikt groter dan b

a is kleiner of gelijk b (ook <= kan)

a b			a is groter of gelijk b (ook >= kan)
...
a.b, axb, a*b		vermenigvuldiging

, a/b			deling

a tot de macht p

vierkantswortel uit a

n-de machtswortel uit a

a			absolute waarde van a

sgn a			signum a: (1 als a>0, 0 als a=0, -1 als a<0)

gemiddelde waarde voor a

n!			n faculteit; n!=n.(n-1).(n-2). … 2.1

binomiaal coëfficiënten;

int a			het grootste geheel getal  a
...
f(b) – f(a) ; gebruikt bij integralen

gf			de samengestelde functie van g en f; g(f(x))

xa			x streeft naar a

limiet van f(x) als x streeft naar a

x			toename van x

Df, f’,		afgeleide van f naar x

Df(a); f’(a); 	waarde van de afgeleide voor x=a

; ;	n-de afgeleide van functie f

partiële afgeleide van functie f naar x

df			differentiaal van functie f; df=f’.dx

de onbepaalde integraal, de verzameling primitieve functies

de bepaalde integraal over het interval [a,b]

\subsubsection{Exponentiële en logaritmische functies}
e			basis van de natuurlijke logaritmen

exponentiële functies met grondtal e

exponentiële functies met grondtal a

logaritme met grondtal a

ln x			natuurlijk logaritme van x

log x			Briggs logaritme van x (grondtal 10)

\subsubsection{goniometrische functies}
verhouding tussen de cirkelomtrek en de middellijn
...
arcsin x			inverse functie van de sinus

arccos x		inverse functie van de cosinus

arctan x		inverse functie van de tangens

arccot x			inverse functie van de cotangens

arcsec x		inverse functie van de secans

arccosec x		inverse functie van de cosecans
...
i, j			i²=-1 (j wordt gebruikt in de electronica)

z = a+bi		het complex getal z

Re z			reëel gedeelte van z

Im z			imaginair gedeelte van z

z			modulus van z

arg z			argument van het getal z

z*			toegevoegd complex getal van z: a-bi

\subsection{Eenheden en hun veelvouden}
De taalkundige regels zijn als volgt. Voluit geschreven prefixen beginnen altijd met kleine letter:

\textit{picofarad, milligram, centimeter, kilovolt, megabyte, gigawatt}.

De afgekorte prefixen hebben een kleine letter tot en met \textit{kilo }en daarboven een hoofdletter: \textit{pF}\textit{, mg, cm, kV, Mb, GW.}

Hier moet goed op gelet worden want het kan grote verschillen in betekenis veroorzaken: de notatie \textit{mW}\textit{ }betekent milliwatt en \textit{MW} Megawatt

Voor de eenheden en grootheden is de regel dat wanneer deze van een persoonsnaam afstammen, zowel voluitgeschreven als afgekort een hoofdletter wordt gebruikt en anders een kleine letter: \textit{Farad, gram, meter, Volt, byte, Watt}.

\subsection{Griekse alfabet}
\section{Het aanpakken van problemen}
\begin{itemize}
\item Maak een tekening, een schets, een schematische voorstelling
\item Proberen met een getallenvoorbeeld / trial and error
\item Werk omgekeerd – werk van achter naar voor
\item Gebruik alle gegevens
\item Splits het probleem op in deelproblemen
\item Stel het probleem voor als opgelost
\item Los een (verwant) eenvoudiger probleem op
\item Zoek een patroon
\item Teken een hulplijn
\item Laat tijdelijk één van de voorwaarden vallen
\item Blikwissel
\end{itemize}
% ==============================
% EINDE: Inhoud geïmporteerd uit Word
% ==============================
